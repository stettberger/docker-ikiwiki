...
sub bibtex_preprocess {
    my %params=@_;
	 ...
	 } elsif ( $output_format eq "publication"){
		  $output=format_publication( $entry, $bibtex_file );
	 } else {
...
sub format_publication() {
	 ...
	 my @display_names;;
	 foreach( @names ){
		  push( @display_names, ($_->part('last'))[0] . ", " . ($_->part('first'))[0] );
	 }
     ...
     my $crossref;
     my $doi;
	 ($title,$year,$journal,$publisher,$crossref, $doi) 
         = $entry->get('title','year', 'journal','publisher', 'crossref', 'doi');
	 if( defined $year ){
		  $output=$output."($year): ";
	 }
     ...
     if (defined $crossref) {
         debug(">> bibtex.pm: $crossref\n");

         my $c_entry = get_bibtex_entry_from_file( $bibtex_file, $crossref );
         my @editor = $c_entry->names('editor');
         my @editor_names;
         my $c_title; my $c_year; my $c_publisher; my $c_address; my $c_venue;
         foreach( @editor ){
             push( @editor_names, ($_->part('last'))[0] . ", " . ($_->part('first'))[0] );
         }
         ($c_title,$c_year, $c_publisher, $c_address, $c_venue)  
             = $c_entry->get('title','year', 'publisher', 'address', 'venue');

         $output .=  "In: ". join("; ", @editor_names ) . ": $c_title ($c_venue) "
	 ...
	 my $pages;
     my $school;
	 ($volume,$number,$pages,$school)=$entry->get('volume','number','pages', 'school');
	 if( defined $volume ){
		  $output=$output."$volume";
	 }
	 if( defined $number ){
	 ...
	 if( defined $doi ){
		  $output.= "<br/> [doi> [$doi](http://dx.doi.org/$doi)]";
	 }
	 if( defined $school ){
         $output.= " $school";
	 }
